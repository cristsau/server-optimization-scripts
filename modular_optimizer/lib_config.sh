#!/bin/bash
# lib_config.sh - Configuration file handling (v1.3)
...
load_config() {
  ...
  if [ -f "$CONFIG_FILE" ]; then
    ...
    if [ $? -eq 0 ]; then source "$CONFIG_FILE"; validate_numeric "${LOCAL_RETENTION_DAYS:-1}" "本地备份保留天数" || LOCAL_RETENTION_DAYS=1; validate_numeric "${REMOTE_RETENTION_DAYS:-1}" "远端备份保留天数" || REMOTE_RETENTION_DAYS=1; [[ "$LOCAL_RETENTION_DAYS" -lt 0 ]] && LOCAL_RETENTION_DAYS=0; [[ "$REMOTE_RETENTION_DAYS" -lt 0 ]] && REMOTE_RETENTION_DAYS=0; export DB_TYPE DB_HOST DB_PORT DB_USER DB_PASS TARGET_PATH TARGET_USER TARGET_PASS LOCAL_RETENTION_DAYS REMOTE_RETENTION_DAYS; log "加载备份配置: $CONFIG_FILE (本地保留: $LOCAL_RETENTION_DAYS 天, 远端保留: $REMOTE_RETENTION_DAYS 天)"; return 0;
...
EOF
   if [ $? -eq 0 ]; then chmod 600 "$CONFIG_FILE" || log "警告: 设置 $CONFIG_FILE 权限失败"; echo "✅ 配置创建/更新成功"; log "备份配置创建/更新成功"; return 0; else echo "❌ 写入配置失败"; log "写入备份配置失败"; return 1; fi
}
...
CLEAN_TMP=true
EOF
    if [ $? -eq 0 ]; then chmod 644 "$OPTIMIZE_CONFIG_FILE" || log "警告: 设置 $OPTIMIZE_CONFIG_FILE 权限失败"; echo "✅ 默认优化配置文件创建成功。"; log "默认优化配置创建成功"; return 0; else echo "❌ 写入优化配置失败。"; log "写入优化配置失败"; return 1; fi
}