...
# --- Backup Config Functions (Renamed back, added retention) ---
load_config() {
  # Set defaults for backup config
  DB_TYPE=""; DB_HOST=""; DB_PORT=""; DB_USER=""; DB_PASS=""; TARGET_PATH=""; TARGET_USER=""; TARGET_PASS="";
  LOCAL_RETENTION_DAYS=1 # Default keep 1 day for local
  REMOTE_RETENTION_DAYS=1 # Default keep 1 day for remote

  if [ -f "$CONFIG_FILE" ]; then # Use CONFIG_FILE
    # Test validity in subshell
    ( source "$CONFIG_FILE" >/dev/null 2>&1 )
    if [ $? -eq 0 ]; then
        source "$CONFIG_FILE" # Source in current shell
        # Validate retention days after loading
        validate_numeric "${LOCAL_RETENTION_DAYS:-1}" "本地备份保留天数" || LOCAL_RETENTION_DAYS=1
        validate_numeric "${REMOTE_RETENTION_DAYS:-1}" "远端备份保留天数" || REMOTE_RETENTION_DAYS=1
        # Ensure non-negative
        [[ "$LOCAL_RETENTION_DAYS" -lt 0 ]] && LOCAL_RETENTION_DAYS=0
        [[ "$REMOTE_RETENTION_DAYS" -lt 0 ]] && REMOTE_RETENTION_DAYS=0

        # Export loaded values for other modules (like lib_backup)
        ...
        log "错误:加载无效备份配置 $CONFIG_FILE"; echo -e "\033[31m✗ 加载备份配置失败\033[0m"; return 1;
    fi
  else
    log "未找到备份配置文件 $CONFIG_FILE"
    # Export defaults even if file not found, so backup function has values
    export LOCAL_RETENTION_DAYS REMOTE_RETENTION_DAYS
    return 1 # Not found
  fi
}

create_config() {
   log "创建/更新备份配置文件: $CONFIG_FILE" # Use CONFIG_FILE
   echo -e "\033[36m▶ 创建/更新备份配置文件 ($CONFIG_FILE)...\033[0m"
   ...
   read -p "数据库类型 (mysql/postgres): " DB_TYPE
   case "$DB_TYPE" in
     mysql) default_port=3306;;
     postgres) default_port=5432;;
     *) echo "类型错误"; return 1;;
   esac
   read -p "数据库主机[127.0.0.1]: " DB_HOST; DB_HOST=${DB_HOST:-127.0.0.1}
   read -p "数据库端口[默认 $default_port]: " DB_PORT; DB_PORT=${DB_PORT:-$default_port}; validate_numeric "$DB_PORT" "端口" || return 1
   read -p "数据库用户: " DB_USER
   read -s -p "数据库密码: " DB_PASS; echo
   read -e -p "备份目标路径: " TARGET_PATH
   ...
   read -p "远端备份保留天数 (0为不删除, 仅WebDAV) [默认 1]: " REMOTE_RETENTION_DAYS; REMOTE_RETENTION_DAYS=${REMOTE_RETENTION_DAYS:-1}; validate_numeric "$REMOTE_RETENTION_DAYS" "远端保留天数" || return 1
   [[ "$LOCAL_RETENTION_DAYS" -lt 0 ]] && LOCAL_RETENTION_DAYS=0 # Ensure non-negative
   [[ "$REMOTE_RETENTION_DAYS" -lt 0 ]] && REMOTE_RETENTION_DAYS=0
...
REMOTE_RETENTION_DAYS="$REMOTE_RETENTION_DAYS" # 主要用于 WebDAV
EOF
   if [ $? -eq 0 ]; then chmod 600 "$CONFIG_FILE"; echo "配置创建/更新成功"; log "备份配置创建/更新成功"; return 0;
   else echo "写入配置失败"; log "写入备份配置失败"; return 1; fi
}

# --- Optimize Config Functions (Use OPTIMIZE_CONFIG_FILE variable) ---
...
create_optimize_config() {
    log "检查/创建优化配置文件: $OPTIMIZE_CONFIG_FILE"
    if [ -f "$OPTIMIZE_CONFIG_FILE" ]; then log "优化配置文件已存在"; return 0; fi
    echo -e "\033[36m▶ 创建默认优化配置文件 ($OPTIMIZE_CONFIG_FILE)...\033[0m"
...
CLEAN_TMP=true
EOF
   if [ $? -eq 0 ]; then chmod 644 "$OPTIMIZE_CONFIG_FILE"; echo "默认优化配置文件创建成功。"; log "默认优化配置创建成功"; return 0;
   else echo "写入优化配置失败。"; log "写入优化配置失败"; return 1; fi
}
