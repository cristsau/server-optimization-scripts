...
install_script() {
    ...
    generated_script_content=$(sed -e "s|__LOG_FILE__|$escaped_log_file|g" -e "s|__CURRENT_VERSION__|$CURRENT_VERSION|g" "$template_file")
    if [ $? -ne 0 ]; then
        log "错误:sed替换模板失败"
        echo "处理模板失败"
        return 1
    ...
    if ! echo "$generated_script_content" | grep -q "LOG_FILE="; then
        log "错误:替换模板变量失败"
        echo "处理模板失败"
        return 1
    fi
    ...
    # 配置主脚本日志轮转
    log "配置主脚本日志轮转: /etc/logrotate.d/setup_optimize_server_main"
    cat > /etc/logrotate.d/setup_optimize_server_main <<EOF
$LOG_FILE {
    rotate 7
    daily
    missingok
