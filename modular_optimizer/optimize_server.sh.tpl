#!/bin/bash
# Generated by cristsau_modular_optimizer v__CURRENT_VERSION__ on $(date) # <-- Updated comment
...
# --- Dependency Check (within generated script) ---
check_dependencies() {
...
clean_docker_logs() {
    if [[ "\$CLEAN_DOCKER_LOGS" != "true" ]]; then log "根据配置跳过 Docker 容器日志清理"; return; fi
    if ! command -v docker &>/dev/null; then log "警告：Docker命令未找到"; return; fi; if ! docker info &>/dev/null; then log "警告：Docker服务未运行"; return; fi; log "清理Docker容器日志..."; local containers container log_path cname; containers=\$(docker ps -a -q); if [ -z "\$containers" ]; then log "无Docker容器"; return; fi; for container in \$containers; do log_path=\$(docker inspect --format='{{.LogPath}}' "\$container" 2>/dev/null); cname=\$(docker inspect --format='{{.Name}}' "\$container" | sed 's/^\///'); if [ -n "\$log_path" ] && [ -f "\$log_path" ]; then log "清理容器(\$cname)日志..."; truncate -s 0 "\$log_path" && log "清理成功" || log "清理失败"; else log "警告：未找到容器(\$cname)日志"; fi; done; log "Docker容器日志清理完成。";
}
clean_apt_cache() { log "清理APT缓存..."; apt-get clean -y >> "\$LOG_FILE" 2>&1; log "APT缓存清理完成。"; }
clean_old_kernels() {
    if [[ "\$CLEAN_OLD_KERNELS" != "true" ]]; then log "根据配置跳过旧内核清理"; return; fi
    log "清理旧内核..."; local current_kernel kernels_to_remove pkg; current_kernel=\$(uname -r); kernels_to_remove=\$(dpkg --list | grep -E '^ii +linux-(image|headers)-[0-9]' | grep -v "\$current_kernel" | awk '{print \$2}'); if [ -n "\$kernels_to_remove" ]; then log "将移除:"; echo "\$kernels_to_remove" | while read pkg; do log "  - \$pkg"; done; apt-get purge -y \$kernels_to_remove >> "\$LOG_FILE" 2>&1; if [ \$? -eq 0 ]; then log "移除成功,清理残留..."; apt-get autoremove -y >> "\$LOG_FILE" 2>&1; log "残留清理完成"; else log "错误：移除失败"; fi; else log "无旧内核可清理"; fi; log "旧内核清理任务结束。";
}
clean_tmp_files() {
    if [[ "\$CLEAN_TMP" != "true" ]]; then log "根据配置跳过 /tmp 清理"; return; fi
    log "清理/tmp目录..."; if [ -d /tmp ]; then find /tmp -mindepth 1 -maxdepth 1 ! -name "optimize_temp.log" -exec rm -rf {} \; 2>> "\$LOG_FILE"; log "临时文件清理完成。"; else log "警告：/tmp不存在"; fi;
}
clean_user_cache() {
    if [[ "\$CLEAN_USER_CACHE" != "true" ]]; then log "根据配置跳过用户缓存清理"; return; fi
    log "清理用户缓存..."; find /home/*/.cache -maxdepth 1 -mindepth 1 \\( -type d -exec rm -rf {} \; -o -type f -delete \\) -print >> "\$LOG_FILE" 2>&1; if [ -d /root/.cache ]; then find /root/.cache -maxdepth 1 -mindepth 1 \\( -type d -exec rm -rf {} \; -o -type f -delete \\) -print >> "\$LOG_FILE" 2>&1; log "清理root缓存完成"; fi; log "用户缓存清理完成。";
}
